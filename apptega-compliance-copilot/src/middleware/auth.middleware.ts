...
import type { UserId, OrganizationId } from '@types/index';
    ...
    before: async (handler: MiddyfiedHandler<APIGatewayProxyEvent, APIGatewayProxyResult>) => {
      ...
      try {
        ...
        if (!authResult.success) {
          ...
          handler.response = unauthorized(authResult.error || 'Authentication required', requestId);
          return;
        }

        if (!authResult.user) {
          handler.response = unauthorized('Invalid authentication', requestId);
          return;
        }
        ...
        if (!authzResult.success) {
          ...
          handler.response = forbidden(authzResult.error || 'Access denied', requestId);
          return;
        }
        ...
        handler.response = unauthorized('Authentication system error', requestId);
      }
    }
  };
};
