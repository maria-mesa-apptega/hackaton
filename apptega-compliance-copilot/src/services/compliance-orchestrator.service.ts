...
export class ComplianceOrchestratorService {
  ...
  private async gatherContext(
    ...
    try {
      ...
      const complianceStatus = this.extractResult(complianceStatusResult, 'compliance status', false);
      const userContext = this.extractResult(userContextResult, 'user context', false);
      const organizationInsights = this.extractResult(organizationInsightsResult, 'organization insights', false);
      const frameworkGuidance = this.extractResult(frameworkGuidanceResult, 'framework guidance', false);
  ...
  private async processWithAI(
    ...
    try {
      ...
      const prompt = this.promptService.buildCompliancePrompt({
        ...
        user: contextData.userComplianceData,
        request,
      ...
      const aiResult = await this.bedrockService.processRequest({
        ...
        userContext: contextData.userComplianceData
      });
  ...
  private buildResponse(
    ...
    const metadata: ComplianceResponseMetadata = {
      dataSources: this.buildDataSources(contextData),
      complianceFrameworks: this.buildFrameworks(request, contextData),
    ...
    if (!promiseResult.value.success) {
      if (required) {
        logger.error(`${operationName} operation failed`, {
          error: promiseResult.value.error.message
        });
      } else {
