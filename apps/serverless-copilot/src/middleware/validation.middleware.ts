...
export const validator = (config: ValidationConfig) => {
  ...
  const middleware = {
    before: async function(handler: MiddyfiedHandler<APIGatewayProxyEvent, APIGatewayProxyResult>) {
      ...
      try {
        ...
        if (validateInput && event.body) {
          ...
          try {
            ...
            (handler as any).response = validationError('Invalid JSON in request body', 'body', requestId);
            return;
          }

          if (!validateInput(parsedBody)) {
            const inputErrors = this.formatAjvErrors((validateInput as any)?.errors || [], 'body');
        ...
        if (validatePathParameters && event.pathParameters) {
          if (!validatePathParameters(event.pathParameters)) {
            const pathErrors = this.formatAjvErrors((validatePathParameters as any)?.errors || [], 'pathParameters');
        ...
        if (validateQueryString && event.queryStringParameters) {
          if (!validateQueryString(event.queryStringParameters)) {
            const queryErrors = this.formatAjvErrors((validateQueryString as any)?.errors || [], 'queryStringParameters');
        ...
        if (validationErrors.length > 0) {
          ...
          (handler as any).response = validationError(
    ...
    formatAjvErrors: function(errors: any[], context: string): ValidationErrorDetail[] {
      return errors.map(error => ({
        field: error.instancePath ? `${context}${error.instancePath}` : context,
        message: this.buildErrorMessage((error as any) || {}),
