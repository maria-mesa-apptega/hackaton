...
export const requestLogger = (config: LoggingConfig = {}) => {
  ...
  return {
    before: async (handler: MiddyfiedHandler<APIGatewayProxyEvent, APIGatewayProxyResult>) => {
      ...
      const context = (handler as any).context;
      ...
      if (finalConfig.logRequests) {
        ...
        logger.info('Incoming request', {
          ...requestInfo,
          organizationId: requestInfo.organizationId?.toString(),
    ...
    after: async (handler: MiddyfiedHandler<APIGatewayProxyEvent, APIGatewayProxyResult>) => {
      ...
      const event = (handler as any).event;
      const context = (handler as any).context;
      const response = (handler as any).response;
    ...
    onError: async (handler: MiddyfiedHandler<APIGatewayProxyEvent, APIGatewayProxyResult>) => {
      ...
      const event = (handler as any).event;
      const context = (handler as any).context;
      const error = (handler as any).error;
...
function extractRequestInfo(
  ...
  if (event.queryStringParameters) {
    (requestInfo as any).queryParams = event.queryStringParameters;
  }

  // Add path parameters
  if (event.pathParameters) {
    (requestInfo as any).pathParams = event.pathParameters;
  }

  // Add body length
  if (event.body) {
    (requestInfo as any).bodyLength = event.body.length;
  }
  ...
  if (config.includeUserInfo) {
    try {
      ...
      if (parsedBody.userId) {
        (requestInfo as any).userId = parsedBody.userId;
      }
      if (parsedBody.organizationId) {
        (requestInfo as any).organizationId = parsedBody.organizationId.toString();
      }
    } catch {
    ...
    if (event.pathParameters?.userId) {
      (requestInfo as any).userId = event.pathParameters.userId;
    }

    // Check query parameters for organization info
    if (event.queryStringParameters?.organizationId) {
      (requestInfo as any).organizationId = parseInt(event.queryStringParameters.organizationId, 10).toString();
    }
  }
